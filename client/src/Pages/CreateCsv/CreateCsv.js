...
class CreateCsv extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.match.params.id) {
      ...
      fetchCurrentJob(this.props.match.params.id).then((data) => {
        this.setState({
          ...this.state,
          job_id: data.job_id,
          section: 3,
  ...
  downloadCSV() {
    downloadCurrentCsv(this.state.data, this.state.exportFileName).then((response) => {});
  }
  ...
  sectionThree() {
    return <div id="section-three">Section Three</div>;
  }
  ...
  render() {
    return (
      <div id="create-csv">
        ...
        {this.state.section === 2 && !this.state.loading ? (
          <SectionTwoContainer
            jobName={this.state.exportFileName}
            data={this.state.data}
            searchZillow={this.state.search_zillow}
            setZillowFlag={this.changeZillow}
            processCSV={this.processCSV}
            selectedFields={this.state.csvExportFields}
            handleAddExportField={this.handleAddExportField}
            handleRemoveExportField={this.handleRemoveExportField}
          />
        ) : (
          ''
        )}
